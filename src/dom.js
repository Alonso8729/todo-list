...
const dom = (() => {
  ...
  const modalDeleteProjectText = document.querySelector(
  ...
  function showMainTitle(index) {
    ...
    iconClasses.forEach((className) => {
      if (className.length > 0) mainTitleIcon.classList.add(className);
    });
    ...
    for (let i = projectIndexStart; i < projectIndexEnd; i++) {
      for (let j = 0; j < projects.projectsList[i].tasks.length; j++) {
        ...
        //Clicked on 'today' - Filter tasks not from today
          ...
          menuCategory === "today" &&
          projects.projectsList[i].tasks[j].date !== todayDate
        ) {
          continue;
        }
        //Clicked on 'completed' - Filter tasks which us incomplete
        else if (
          menuCategory === "completed" &&
          projects.projectsList[i].tasks[j].completed === false
        ) {
          continue;
        }
        //Clicked on 'week' - Filter tasks which doesn't take place at the following week
          ...
          if (
            !(
              differenceInDays(taskDate, dateOfToday) <= 7 &&
              differenceInDays(taskDate, dateOfToday) >= 0
            )
          ) {
            continue;
          }
    ...
    if (!form.classList.contains("hide") && formTitle.value === "") {
      ...
      if (modalTaskDiv.classList.contains("hide")) {
        ...
        const newProjectIndex =
          projectList.lastChild.getAttribute("data-link-index");
  ...
  function selectLink(target, index, action) {
    ...
    //Clicked somewhere on project-link
      ...
      if (
        target.classList.contains("left-project-div") ||
        target.classList.contains("project-link-icons")
      ) {
